...
trait UserAttributes
{
    ...
    {
        $name = $class == '' ? 'Zaloguj się jako' : '';
        // If the admin is currently NOT spoofing a user
        if (auth()->user()->can('login-as-user') && (! session()->has('admin_user_id') || ! session()->has('temp_user_id'))) {
            //Won't break, but don't let them "Login As" themselves
            if ($this->id != auth()->user()->id) {
                echo
                '<a class="btn btn-success" href="'.url('admin.auth.user.login-as', $this).'">
                    <i class="fa fa-lock" data-toggle="tooltip" data-placement="top" title="'.trans('buttons.backend.auth.users.login_as', ['user' => $this->name]).'"></i>
                    '.$name.'
                </a>';
            }
        }
    ...
    {
        if ($this->id != auth()->id() && auth()->user()->allow('delete-user')) {
    ...
    public function getStatusButtonAttribute($class)
    {
        if ($this->id != auth()->id()) {
            switch ($this->status) {
                case 0:
                    if (auth()->user()->can('activate-user')) {
                        $name = $class == '' ? 'Aktywuj konto' : '';
                        echo '
                        <a class="btn btn-success" href="'.url('admin.auth.user.mark').'">
                            <i class="fa fa-check-square" data-toggle="tooltip" data-placement="top" title="'.$name.'"></i> '.$name.'
                        </a>';
                    }
                ...
                case 1:
                    if (auth()->user()->can('deactivate-user')) {
                        $name = ($class == '') ? 'Dezaktywuj konto ' : '';
                        echo '
                        <a class="btn btn-danger" href="'.url('admin.auth.user.mark').'">
                            <i class="fa fa-check-square" data-toggle="tooltip" data-placement="top" title="'.$name.'"></i> '.$name.'
                        </a>';
                    }
    ...
    public function getClearSessionButtonAttribute($class)
    {
        ...
        if ($this->id != auth()->user()->id && config('session.driver') == 'database' && auth()->user()->can('clear-user-session')) {
            echo '
            <a href="'.url('admin.auth.user.clear-session', $this).'" class="btn btn-danger" name="confirm_item">
